...
open `arduino-ide/smart-dryer-single-file/smart-dryer-single-file.ino` →
...
- ESP32 DevKit V1
- The `smart-dryer` folder from this repo (the one containing `platformio.ini`)
...
- File → Open Folder… → select **`smart-dryer`** (the folder that directly
...
[diag] --------------------------------
[ota] network OTA ready - Arduino IDE: Tools > Port > smart-dryer at 192.168.4.1
