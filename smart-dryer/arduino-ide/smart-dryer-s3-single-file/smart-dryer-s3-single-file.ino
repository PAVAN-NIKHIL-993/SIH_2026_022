 ...
 *    GPIO 8   I2C0 SDA  - AHT10 #1 + keypad backpack
 *    GPIO 9   I2C0 SCL
 *    GPIO 1   HX711 weigh scale PD_SCK
 *    GPIO 2   HX711 weigh scale DOUT
 *    GPIO12   BTS7960 RPWM (via the 555 level shifter)
 *    GPIO13   BTS7960 R_EN + L_EN (jumpered)
 *    GPIO17   L298N ENB - the ONE outlet fan PWM
 *    GPIO 4   battery divider -> ADC
 *    GPIO 5   battery divider gate (2N7000)
 *    GPIO 6   supply relay CH1 - solar feed
 *    GPIO 7   supply relay CH2 - bypass feed
 *    GPIO39   SOLAR/BYPASS toggle (HIGH = solar)
 *    GPIO18   supply optocoupler (HIGH = feed live)
 *    GPIO14   P-MOSFET latch hold (hard power-off)
 *    GPIO15   BUTTON-1: 3 s hard off / 10 s reboot
 *    GPIO16   BUTTON-2: default automation
 *    GPIO21   door limit switch / reed (closed = LOW)
 *    GPIO38   buzzer KY-012
 ...
 *    GPIO40   DS1302 RTC RST (chip enable) - date & time, coin-cell
 *    GPIO42   DS1302 RTC SCLK
 *    GPIO47   DS1302 RTC data I/O
...
static void drawMenu() {
  ...
  if (menu::info()) {                       // the OTA firmware-update screen
    ...
    drawText(16, 282, "  Port -> smart-dryer", 2, C_GOLD, C_NAVY);
...
static void openEdit() {
  ...
  if (sCur == R_OTA) {                      // OTA info screen (spec)
    ...
    Serial.println(F("[menu] OTA firmware update: WiFi AgarbattiDryer -> "
                     ...
                     "-> Port -> smart-dryer at 192.168.4.1"));
    return;
  }
...
#if OTA_NETWORK_ENABLED
  // 4b. network OTA: Arduino IDE -> Tools > Port -> "smart-dryer at
...
#endif  // DRYER_RTOS
/* ==== END OF FILE ====
 * total lines (wc -l): 8912   non-blank lines: 8238
